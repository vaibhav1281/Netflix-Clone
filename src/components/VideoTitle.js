...
const VideoTitle = ({title, overview}) => {
    // console.log(title, overview) 01:19:35
  return (
    <div className='pt-[20%] px-12 absolute text-white bg-gradient-to-r from-black w-full aspect-video'>
        <h1 className='text-5xl font-bold mb-4'>{title}</h1>
        <p className='py-6 text-lg w-1/4 flex max-sm:hidden'>{overview}</p>
        <div className='flex gap-2 items-center'>
            <button className='flex bg-white font-semibold gap-1 p-1 rounded-md px-3 text-black hover:opacity-55'>
              <span>
