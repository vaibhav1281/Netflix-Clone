...
const VideoTitle = ({title, overview}) => {
  return (
    <div className='pt-[20%] px-12 absolute text-white bg-gradient-to-r from-black w-full aspect-video'>
        <h1 className='lg:text-5xl md:text-3xl max-sm:text-2xl max-sm:font-normal font-bold mb-4 max-sm:mt-[25%]'>{title}</h1>
        <p className='py-6 text-lg lg:text-lg md:text-base sm:text-sm w-1/4 lg:w-2/4 sm:w-3/4 flex max-sm:hidden'>{overview}</p>
        <div className='flex gap-2 items-center'>
            <button className='flex bg-white font-semibold gap-1 p-1 rounded-md px-3 max-sm:px-1 text-black hover:opacity-55 max-sm:hidden'>
              <span>
            ...
            <button className='flex bg-gray-400 p-1 rounded-md font-semibold gap-1 max-sm:hidden'>
              <span>
